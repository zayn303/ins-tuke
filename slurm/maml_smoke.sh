#!/bin/bash
#SBATCH --job-name=pd-dg-maml-smoke
...
exec >"${LOG_DIR}/maml_${MODEL}_held${HELD_OUT}_${IDX}.out" 2>"${LOG_DIR}/maml_${MODEL}_held${HELD_OUT}_${IDX}.err"
...
echo "Running: method=maml model=${MODEL} held_out_domain=${HELD_OUT} epochs=1 n_episodes_per_epoch=2 [SMOKE]"
