...
# Submit with:
#   sbatch --export=ALL,RUN_TS=$(date +%Y-%m-%d_%H%M),HF_HOME=/home/ak562fx/.cache/huggingface slurm/coral_8gpu.sh
...
run_one() {
    ...
    echo "[start] GPU${GPU_ID}: coral/${MODEL}/held${HELD} (idx=${IDX})"
    CUDA_VISIBLE_DEVICES=${GPU_ID} python scripts/train.py \
        ...
        wandb_offline=true \
        num_workers=4 \
        >"${LOG_DIR}/coral_${MODEL}_held${HELD}_${IDX}.out" \
        2>"${LOG_DIR}/coral_${MODEL}_held${HELD}_${IDX}.err"
    local ec=$?
    echo "[done]  GPU${GPU_ID}: coral/${MODEL}/held${HELD} (idx=${IDX}) exit=${ec}"
    return $ec
}
...
echo "=== All 12 CORAL jobs complete ==="
