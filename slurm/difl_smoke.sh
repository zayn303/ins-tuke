#!/bin/bash
#SBATCH --job-name=pd-dg-difl-smoke
...
# Smoke test: 1 epoch per task, 15 variants (3 models x 5 held-out domains). Run before full difl_array.sh.
# RUN_TS and HF_HOME auto-generated if not exported. To override:
#   sbatch --export=ALL,RUN_TS=$(date +%Y-%m-%d_%H%M),HF_HOME=/home/ak562fx/.cache/huggingface slurm/difl_smoke.sh
...
echo "Running: method=difl model=${MODEL} held_out_domain=${HELD_OUT} epochs=1 [SMOKE]"
