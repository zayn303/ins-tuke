...
# Submit with:
#   sbatch --export=ALL,RUN_TS=$(date +%Y-%m-%d_%H%M),HF_HOME=/home/ak562fx/.cache/huggingface slurm/erm_smoke_8gpu.sh
...
echo "RUN_DIR=${RUN_DIR} (smoke: 1 epoch)"
...
# 15 jobs total: 5 folds × 3 models — same ordering as erm_8gpu.sh
JOBS=(
...
echo "=== Smoke complete: ${total_fail}/15 failed ==="
if ((total_fail > 0)); then
    echo "FAIL: ${total_fail} jobs failed — check .err logs in ${LOG_DIR}/"
    exit 1
fi
echo "PASS: all 15 smoke jobs exit 0"
