#!/bin/bash
#SBATCH --job-name=pd-dg-erm-smoke
...
# Smoke test: 1 epoch per task, all 12 variants. Run before full erm_array.sh.
# Submit with:
#   sbatch --export=ALL,RUN_TS=$(date +%Y-%m-%d_%H%M),HF_HOME=/home/ak562fx/.cache/huggingface slurm/erm_smoke.sh
...
exec >"${LOG_DIR}/erm_${MODEL}_held${HELD_OUT}_${IDX}.out" 2>"${LOG_DIR}/erm_${MODEL}_held${HELD_OUT}_${IDX}.err"
...
echo "Running: method=erm model=${MODEL} held_out_domain=${HELD_OUT} epochs=1 [SMOKE]"
