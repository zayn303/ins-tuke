"""Run all 15 CORAL smoke tests (1 epoch, 3 parallel workers by default).

Usage:
    python scripts/smoke_coral.py                       # 3 workers default (one per backbone)
    python scripts/smoke_coral.py --workers 15          # fully parallel (OOM risk on single GPU)
    python scripts/smoke_coral.py --workers 1           # sequential
    python scripts/smoke_coral.py --data-root /home/ak562fx/ins-tuke/Data
    python scripts/smoke_coral.py --batch-size 2        # reduce VRAM further
"""
import argparse
import os
import subprocess
...
def run_combo(model, held_out, log_dir, ckpt_dir, data_root, batch_size, num_workers):
    tag = f"{METHOD}_{model}_held{held_out}"
...
def main():
    ...
    log_dir = ROOT / "logs" / f"{ts}_{METHOD}_smoke"
    ckpt_dir = ROOT / "checkpoints" / f"{ts}_{METHOD}_smoke"
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            try:
                ...
                m, h = futures[fut]
                tag = f"{METHOD}_{m}_held{h}"
                results.append((tag, False, f"ERROR({repr(e)})"))
                seen_tags.add(tag)

    expected = {f"{METHOD}_{m}_held{h}" for h in HELD_OUTS for m in MODELS}
