...
Usage:
    ...
    python scripts/smoke_coral.py --workers 1
    python scripts/smoke_coral.py --data-root /home/ak562fx/ins-tuke/Data
    python scripts/smoke_coral.py --batch-size 2
"""
import argparse
import os
import subprocess
...
def run_combo(model, held_out, log_dir, ckpt_dir, data_root, batch_size, num_workers):
    tag = f"coral_{model}_held{held_out}"
    cmd = [
...
def main():
    ...
    log_dir = ROOT / "logs" / f"{ts}_coral_smoke"
    ckpt_dir = ROOT / "checkpoints" / f"{ts}_coral_smoke"
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            try:
                ...
                m, h = futures[fut]
                tag = f"coral_{m}_held{h}"
                ok, status = False, f"ERROR({repr(e)})"
            results.append((tag, ok, status))
            seen_tags.add(tag)
    for tag in sorted({f"coral_{m}_held{h}" for h in HELD_OUTS for m in MODELS} - seen_tags):
