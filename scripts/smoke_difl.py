...
Usage:
    python scripts/smoke_difl.py
    python scripts/smoke_difl.py --workers 1
    python scripts/smoke_difl.py --data-root /home/ak562fx/ins-tuke/Data
    python scripts/smoke_difl.py --batch-size 2
"""
import argparse
import os
import subprocess
...
def run_combo(model, held_out, log_dir, ckpt_dir, data_root, batch_size, num_workers):
    tag = f"difl_{model}_held{held_out}"
    cmd = [
...
def main():
    ...
    log_dir = ROOT / "logs" / f"{ts}_difl_smoke"
    ckpt_dir = ROOT / "checkpoints" / f"{ts}_difl_smoke"
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            try:
                ...
                m, h = futures[fut]
                tag = f"difl_{m}_held{h}"
                ok, status = False, f"ERROR({repr(e)})"
            results.append((tag, ok, status))
            seen_tags.add(tag)
    for tag in sorted({f"difl_{m}_held{h}" for h in HELD_OUTS for m in MODELS} - seen_tags):
