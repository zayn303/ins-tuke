"""Run all 15 MAML smoke tests (1 epoch, 3 parallel workers by default).

Usage:
    python scripts/smoke_maml.py                       # 3 workers default (one per backbone)
    python scripts/smoke_maml.py --workers 15          # fully parallel (OOM risk on single GPU)
    python scripts/smoke_maml.py --workers 1           # sequential
    python scripts/smoke_maml.py --data-root /home/ak562fx/ins-tuke/Data
    python scripts/smoke_maml.py --batch-size 2        # reduce VRAM further
"""
import argparse
import os
import subprocess
